...
const CreatePost = () => {
    ...
    if (form.prompt) {
      try {
        ...
        const response = await fetch(
          "https://dalleclone-0zjt.onrender.com/api/v1/dalle",
          {
            method: "POST",
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch(
          "https://dalleclone-0zjt.onrender.com/api/v1/posts",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <section className="nax-w-7xl mx-auto">
      <div>
        <h1 className="font-extrabold text-[#222328] text-[32px]">Create</h1>
        <p className="mt-2 text-[#666e75] text-[18px] max-w[500px]">
          Create a Imagine through DALLE-AI and share them with people online
        </p>
      </div>
